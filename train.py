...
if is_wandb_available():
    import wandb
    #api_key = "92c0eec4d8cb43f0e5e16713af60380fbf3e2827"
...
class DreamBoothDataset(Dataset):
            ...
            if path.suffix.lower() in [".jpg", ".jpeg", ".png"]:
                ...
                if caption_path.exists():
                    with open(caption_path, "r") as f:
                        instance_captions.append(f.read().strip())
                else:
    ...
    def __getitem__(self, index):
        ...
        image_hash = self.generate_image_hash(self.instance_images[index % self.num_instance_images].filename)
        ...
        example["instance_captions"] = instance_caption
        return example
...
def collate_fn(examples):
    ...
    instance_captions = [example["instance_captions"] for example in examples]
    ...
    batch = {
        ...
        "instance_captions": instance_captions,
    }
    return batch
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % num_update_steps_per_epoch == 0:
                        if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                            ...
                            if args.mixed_precision == "fp16":
                                models = [pipeline.transformer]
                                cast_training_params(models, dtype=torch.float16)
                            elif args.mixed_precision == "bf16":
                                models = [pipeline.transformer]
                                cast_training_params(models, dtype=torch.bfloat16)
                            else:
                                models = [pipeline.transformer]
                                cast_training_params(models, dtype=torch.float32)
        ...
        if args.push_to_hub and epoch < args.num_train_epochs - 1:
            accelerator.push_to_hub(repo_id, commit_message=f"Training in progress epoch {epoch}", blocking=False)
    ...
    if accelerator.is_main_process:
        ...
        if args.push_to_hub:
            ...
            #accelerator.push_to_hub(repo_id, commit_message="End of training")
            api = HfApi()
            repo = Repository(args.output_dir, clone_from=args.hub_model_id)
            repo.git_add(auto_lfs_track=True)  # Track large files automatically with Git LFS
            repo.git_commit("End of training")
            repo.git_push()
